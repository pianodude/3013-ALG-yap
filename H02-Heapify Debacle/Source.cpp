...
// Label:            07-H02
// Title:            Heapify Debacle
// Course:           CMPS 3013
// Semester:         Spring 2020
//
// Description:
//       demonstrates Min Heap
//
// Usage:
//       
//
 ...
 *      private:
 *          BubbleUp    : swaps from child to parent
 *          Left        : left child 2i
 *          OnHeap      : checks if i is on heap
 *          Parent      : i/2
 *          Right       : right child 2i+1
 *          Swap        : swaps two values in heap
 *          /// Fix These:
 *          SinkDown    : swaps from parent to child
 *          PickChild   : checks if either left or right is smaller
 *      public:
...
private:
   ...
   */
	...
	void Heapify(int A*, int size) {
		...
		int called = 0;
		//loops if there is a swap
		while (called != 0) 
		{
			called = 0; //pass by reference
			for (int i = 1; i <= (size / 2); i++) //normally stop at half
			{
				SinkDown(i, called);
	 ...
	 * @param  {int}& called : counter if the swap is made
	 * @return              : void
	 */
	void SinkDown(int index, int& called) {
		//do stuff!
